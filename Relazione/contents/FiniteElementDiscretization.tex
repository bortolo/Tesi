...
We notice that presently meshing software are oriented to care about the minimum angle of the elements, rather than the maximum, as a conseguence obtain a mesh which satisfied the condition \referenzaeq{eq: mesh delaunay condition} it's a really difficult task. 

As example in \figref{fig: cubo zikatanov} we present a simple partition of a cube performed with the Synopsis tool SNMESH. For every element we evaluate how many edges doesn't satisfy the condition \referenzaeq{eq: mesh delaunay condition}.
...
\begin{figure}[!h]
\centering
\includegraphics[height = 7cm]{FiniteElementDiscretization/CuboElementi.png}
