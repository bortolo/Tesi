...
\figref{fig: density occupation and prob} shows  the state density occupation $g(E)$ and the probability distribution $f(E)$ for a metal and a semiconductor. Because the emission of electrons are located mainly at the bottom of the conduction band, in order to obtain an analytic formula for the state density occupation $g(E)$, we can consider the following parabolic approximation of the conduction band
...
In general \referenzaeq{eq: carrier densiy integral} is a Fermi integral of order $1/2$ and must be evaluated numerically.
...
In most cases, when the energy is at least several $k_BT$ above or below the Fermi level (case of non degenerate semiconductor), equation \referenzaeq{eq: fermi dirac distribution} can be well approximated by the Maxwell-Boltzmann statistics for classical particles, which reads as follows:
...
Formulas similar to \referenzaeq{eq: carrier densiy integral} and \referenzaeq{eq: state density electron} can be used to compute the hole concentration $p$ in the valence band and find
...
In an intrinsic semiconductor $n=p$ and the \textit{intrinsic Fermi level} $E_i$ can be calculated using equations \referenzaeq{eq: n density fd} and \referenzaeq{eq: p density fd} to get:
...
By replacing \referenzaeq{eq: midgap equilibrium} in \referenzaeq{eq: n density fd} we determine the expression of the intrinsic carrier concentration $n_i=n=p$
...
The Fermi level in n-type silicon moves up towards the conduction band while in p-type silicon it moves down towards the valence band. This behaviour is well depicted by the band diagrams presented in \figref{fig: Band diagrams of extrinsic silicon}.
...
\end{equation}
 ...
 Because the magnitude of impurities is between $10^{16}\div 10^{20} [cm^{-3}]$, and intrinsic carrier concentration in the order of $10^{10}[cm^{-3}]$, in typical case we can approximate the carrier concentrations as:
...
Applying the divergence operator to \referenzaeq{eq: magnetic equation} we get the \textit{Continuity Equation}
...
To close system \referenzaeq{eq: Poisson equation} \referenzaeq{eq: continuity equation total}, we need to specify the mathematical form of the electric charge density ($\rho$) and the electric conduction current density ($\vect{J}$).
