...
A suitable formulation of che carrier concentration is given, for electrons, by the follow integral:
\begin{equation}
...
In most cases when the energy is at least several $k_BT$ above or below the Fermi level, equation \referenzaeq{eq: fermi dirac distribution} can be approximated with the Maxwell-Boltzmann statistics for classical particles, which reads as follows:
...
\end{equation}
 ...
 Take into account this approximation and substituting \referenzaeq{eq: n density fd} and \referenzaeq{eq: p density fd} in \referenzaeq{eq: equilibrium charge in n-type} and \referenzaeq{eq: equilibrium charge in p-type}, solving the algebraic equation have
