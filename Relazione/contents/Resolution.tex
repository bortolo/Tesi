...
where $D := N_D-N_A$ is the net doping concentration. Furthermore  we can approximate that the quasi fermi potential level of silicon at contact is aligend with the external applyed voltage. As a conseguence we can easily calculate potential condition on $\Gamma_{D,Si}$ using \referenzaeq{eq: n density mb} and \referenzaeq{eq: p density mb}
